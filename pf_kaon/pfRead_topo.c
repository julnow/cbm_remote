...
void pfRead_initial(const char *fileName, const char *outputSignName, const char *ouputBckgrName){
  ...
  //chi2
  TH1F hchi2_geo("hchi2_geo", "#chi^{2} geometric; #chi^{2}_{geo}; counts", 100, -1, 20);
  ...
  TH1F hchi2_topo("hchi2_topo", "#chi^{2} topological; #chi^{2}_{topo}; counts", 100, -1, 25);
  ...
  TH1F hcosine_topo("hcosine_topo", "cosine topological; cos_{topo}; counts", 200, .9, 1);
  TH1F hdistance("hdistance", "DCA between secondary tracks; DCA_{#pi_{+}#pi_{-}} [cm]; counts", 100, 0, 1.5);
