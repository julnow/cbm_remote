...
for ((i=1+50*(INDEX-1); i<=50*INDEX; i++)); do
  echo /lustre/cbm/users/lubynets/cbm2atree/outputs/apr20_fr_18.2.1_fs_jun19p1/dcmqgsm_smm_pluto/auau/12agev/mbias/sis100_electron_target_25_mkm/AT2/$i/$i.analysistree.root >> filelist_${INDEX}.txt
done
mkdir -p $PWD/pfsimple/${INDEX}
cd $PWD/pfsimple/${INDEX}
#run pf_simple on each file
$pfpath filelist_${INDEX}.txt
